...
public class AuthController {
    ...
    public ResponseEntity<?> register(@Valid @RequestBody RegisterReq req){
        ...
        catch (Exception e){ return ResponseEntity.badRequest().body(Map.of("message", "Nepavyko sukurti paskyros")); }
    }
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginReq req){
        ...
        catch(Exception ex){ return ResponseEntity.status(401).body(msg("Nepavyko prisijungti")); }
    }

    @PostMapping("/logout")
    public ResponseEntity<?> logout(@RequestHeader(value="Authorization",required=false) String authz){
        try { auth.logout(authz!=null && authz.startsWith("Bearer ") ? authz.substring(7) : ""); return ResponseEntity.ok(msg("Atsijungta")); }
        catch(Exception e){ return ResponseEntity.ok(msg("Atsijungta")); }
    }
    ...
    public ResponseEntity<?> me(@RequestHeader("Authorization") String authz){
        ...
        return ResponseEntity.ok().body(java.util.Map.of(
                ...
                "units", u.getUnits(), "theme", u.getTheme(),
                "dailyCalories", u.getDailyCalories(), "autoAddAi", u.isAutoAddAi()
        ));
    }

    @PutMapping("/me")
    public ResponseEntity<?> updateMe(@RequestHeader("Authorization") String authz,
    ...
    private static java.util.Map<String,String> msg(String m){ return java.util.Map.of("message", m); }
    private static java.util.Map<String,String> msg(Exception e){ return msg(e.getMessage()!=null?e.getMessage():"Klaida"); }
}
